...
def create_svd_docs_similarity(recipes):
    ...
    words_compressed, _, docs_compressed = svds(td_matrix, k=60)
    docs_compressed
    ...
    return documents, word_to_index, index_to_word, words_compressed_normed, docs_compressed_normed

def closest_words(documents, word_to_index, index_to_word, words_compressed_normed, docs_compressed_normed, word_in, k = 5):
    if word_in not in word_to_index: return "Not in vocab."
    sims = words_compressed_normed.dot(words_compressed_normed[word_to_index[word_in],:])
    asort = np.argsort(-sims)[:k+1]
    return [(index_to_word[i],sims[i]) for i in asort[1:]]

def closest_recipes_to_word(documents, word_to_index, index_to_word, words_compressed_normed, docs_compressed_normed, word_in, k = 5):
    if word_in not in word_to_index: return "Not in vocab."
